...
class QueriesTest < ActionDispatch::IntegrationTest
  ...
  def test_create
    ...
    post blazer.run_queries_path,
         params: { statement: query.statement, data_source: query.data_source, query_id: query.id }, xhr: true
    assert_response :success
  ...
  def test_variables_time_range
    ...
    get blazer.query_path(query)
    assert_response :success
    assert_match %(name="start_time"), response.body
    assert_match %(name="end_time"), response.body
  end
  ...
  def test_csv
    ...
    assert_equal "attachment; filename=\"query.csv\"; filename*=UTF-8''query.csv",
  ...
  def test_csv_query
    ...
    assert_equal "id,city\n1,Chicago\n", response.body
    assert_equal "attachment; filename=\"all-cities.csv\"; filename*=UTF-8''all-cities.csv",
                 response.headers['Content-Disposition']
    assert_equal 'text/csv; charset=utf-8', response.headers['Content-Type']
  end
  ...
  def test_csv_query_variables
    ...
    assert_equal "id,city\n1,Chicago\n", response.body
    assert_equal "attachment; filename=\"cities.csv\"; filename*=UTF-8''cities.csv",
                 response.headers['Content-Disposition']
    assert_equal 'text/csv; charset=utf-8', response.headers['Content-Type']
  end
